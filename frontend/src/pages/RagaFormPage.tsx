...
export default function RagaFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) { await updateRaga(id!, payload); navigate(`/ragas/${id}`) }
      else { const { data } = await createRaga(payload); navigate(`/ragas/${data.id}`) }
    } catch (err: any) {
      ...
      <div style={{ maxWidth: '680px' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="sv-card mb-5 p-6">
            ...
            {!isEdit && <p style={{ marginTop: '0.75rem', fontSize: '0.78rem', color: 'rgba(240,228,200,0.3)', fontStyle: 'italic' }}>Audio uploads become available after saving.</p>}
          </div>
